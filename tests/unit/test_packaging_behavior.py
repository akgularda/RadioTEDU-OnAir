...
def test_release_bundle_cli_emits_manifest_json(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, str(script), "2.4.6"],
        cwd=root,
        capture_output=True,
        text=True,
    )
...
def test_build_backend_onefile_includes_passlib_bcrypt_hidden_import():
    ...
    assert '$pyInstallerDistRoot = ".\\build\\backend-publish"' in backend_script
    assert '$pyInstallerWorkRoot = ".\\build\\pyinstaller-work"' in backend_script
