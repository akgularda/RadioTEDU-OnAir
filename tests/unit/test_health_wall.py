...
def test_health_wall_static_page_has_no_controls_or_login():
    with TestClient(app, client=("127.0.0.1", 50000)) as client:
        page = client.get("/static/health-wall/index.html")
        script = client.get("/static/health-wall/health-wall.js")

    assert page.status_code == script.status_code == 200
    assert 'id="wallTitle"' in page.text
    assert "<button" not in page.text
    assert "<form" not in page.text
    assert "/api/monitor/snapshot" in script.text
    assert "render(staleSnapshot(model.snapshot))" in script.text
    assert ".post(" not in script.text.lower()
    assert "method: 'post'" not in script.text.lower()
    assert "physical_device" in script.text
    assert "getusermedia" not in script.text.lower()
