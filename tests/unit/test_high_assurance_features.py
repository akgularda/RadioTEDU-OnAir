...
def test_stream_wizard_markup_is_plain_language_and_accessible():
    ...
    html = (root / "app" / "static" / "deterministic-wall" / "index.html").read_text(encoding="utf-8")
    script = (root / "app" / "static" / "deterministic-wall" / "app.js").read_text(encoding="utf-8")
    for step in range(1, 5):
        assert f"STEP {step} OF 4" in html
    assert "Most compatible — MP3 128 kbps" in html
    assert "Higher quality — AAC+ 196 kbps" in html
