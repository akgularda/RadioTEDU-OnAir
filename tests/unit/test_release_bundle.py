...
def test_release_manifest_contains_backend_and_desktop_artifacts():
    manifest = build_release_manifest("1.0.0")
    ...
    assert artifacts["RadioTEDU-OnAir-Agent.exe"] == "dist/desktop/RadioTEDU-OnAir-Agent.exe"
...
def test_bundle_scripts_pin_release_layout_and_local_dotnet():
    ...
    assert "dist\\desktop" in bundle_script
    ...
    assert 'Join-Path $root "last_build_path.txt"' in bundle_script
...
def test_backend_bundle_script_builds_console_backend_bundle_into_backend_dist():
    ...
    assert "launched hidden by the desktop agent" in backend_script
...
if ($Args.Count -ge 1 -and $Args[0] -eq 'publish') {
    ...
    $exeName = if ($projectPath -like '*Shell*') { 'CleanroomRadio.Shell.exe' } else { 'CleanroomRadio.Agent.exe' }
...
""".strip()
    ...
    assert (script_root / "dist" / "desktop" / "RadioTEDU-OnAir-Agent.exe").exists()
    assert (
