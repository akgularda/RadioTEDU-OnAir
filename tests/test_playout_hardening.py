...
class PlayoutHardeningTests(unittest.TestCase):
    ...
    def test_radio_mount_repair_only_updates_exact_radio_mount(self):
        ...
        repaired = app_db._repair_station_output_mount_typos(cur)

        self.assertTrue(repaired)
        outputs = dict(cur.execute("SELECT station_id, icecast_mount FROM station_outputs"))
        self.assertEqual(outputs[1], "/radiotedu1")
        self.assertEqual(outputs[2], "/lofi")
        self.assertEqual(outputs[3], "/radio1")
        settings = {
            (row[0], row[1]): row[2]
            for row in cur.execute("SELECT station_id, key, value FROM station_settings")
        }
        self.assertEqual(settings[(1, "icecast_mount")], "/radiotedu1")
        self.assertEqual(
            settings[(1, "icecast_url")],
            "http://stream.example.test:8000/radiotedu1",
        )
        self.assertEqual(settings[(2, "icecast_mount")], "/lofi")
        self.assertEqual(settings[(2, "icecast_url")], "http://stream.example.test:8000/lofi")
        self.assertEqual(settings[(3, "icecast_mount")], "/radio1")

    def _read_app_js(self):
        return source_file("app", "static", "js", "app.js").read_text(
            encoding="utf-8"
        )
    ...
    def test_frontend_checks_health_before_websocket_connects(self):
        ...
        boot_anchor = app_js.index("syncPanelVisibilityUi();")
        first_ws_connect = app_js.index("WS.connect();", boot_anchor)
        first_health_refresh = app_js.index("await refreshHealth();", boot_anchor)

        self.assertLess(first_health_refresh, first_ws_connect)
        reconnect_positions = []
        offset = 0
        while True:
            pos = app_js.find("WS.connect(true);", offset)
            if pos < 0:
                break
            reconnect_positions.append(pos)
            offset = pos + 1
        self.assertGreaterEqual(len(reconnect_positions), 2)
        for pos in reconnect_positions:
            self.assertIn("await refreshHealth();", app_js[max(0, pos - 120):pos])

    def test_frontend_show_controls_use_parsed_api_fetch_payloads(self):
        app_js = self._read_app_js()

        users_loader = self._extract_js_function(app_js, "loadAssignableUsers")
        self.assertIn("Array.isArray(data?.items) ? data.items : []", users_loader)

        for name in ("handleGoLive", "handleGoBreak", "handleEndShow"):
            handler = self._extract_js_function(app_js, name)
            self.assertIn("const data = await apiFetch", handler)
            self.assertIn("data?.session || null", handler)
            self.assertNotIn(".ok", handler)
            self.assertNotIn(".json(", handler)
    ...
    def test_frontend_clean_scan_uses_entered_folder(self):
        app_js = self._read_app_js()
        body = self._extract_js_function(app_js, "triggerScanAndTrim")

        self.assertIn("scanFolderPath", body)
        self.assertIn("scanFolderRecursive", body)
        self.assertIn("params.set('folder', folder)", body)
        self.assertIn("trim_silence: 'true'", body)
        self.assertIn("clean_intro: 'true'", body)
    ...
    def test_station_settings_default_to_aac_while_offering_fallback_profiles(self):
        index_html = source_file("app", "static", "index.html").read_text(
            encoding="utf-8"
        )
        app_js = self._read_app_js()
        legacy_py = source_file("app", "api", "legacy.py").read_text(
        ...
        self.assertIn('value="opus_196"', index_html)
        self.assertIn('value="opus_320"', index_html)
        self.assertIn('value="aac_192" data-bitrate="192" selected', index_html)
        self.assertIn('value="aac_plus_196"', index_html)
        save_station_settings = self._extract_js_function(app_js, "saveStationSettings")
        self.assertIn("stStreamCodecProfile", save_station_settings)
        self.assertIn("|| 'aac_192'", save_station_settings)
        self.assertIn("stream_codec_profile: streamCodecProfile", save_station_settings)
    ...
    def test_icecast_sink_preserves_running_reconnecting_source_client(self):
        ...
        sink = IcecastAudioSink("ffmpeg", spawn)
