...
def _ai_startup_status_snapshot(station_id: int, station_settings: dict) -> dict:
    ...
    try:
        ...
        target = startup_buffer_target(station_settings)
        readiness = get_ai_prefetch().readiness_snapshot(
            int(station_id),
            lookahead=max(1, int(target)),
        )
        ready_count = _safe_int(readiness.get("ready_track_intros", 0), 0)
        required_count = max(1, int(target))
        return {
            "ai_startup_state": "ready" if ready_count >= required_count else "warming",
        ...
        required_count = _safe_int(station_settings.get("startup_ai_required_intro_count", 0), 0)
        return {
            "ai_startup_state": state or "disabled",
            "ai_ready_intro_count": ready_count,
            "ai_required_intro_count": required_count,
        }
...
def _send_icecast_metadata(
    ...
    *,
    ...
    outputs.extend(
        ...
        if bool(dict(output).get("enabled", True))
    )
    attempted = False
    all_sent = True
        ...
        outputs.append(
            {
                ...
                "stream_codec_profile": str(
                    value.get("stream_codec_profile") or value.get("codec") or row["stream_codec_profile"]
                ),
                "stream_bitrate_kbps": _safe_int(
                    value.get("stream_bitrate_kbps", value.get("bitrate_kbps", row["stream_bitrate_kbps"])),
                    int(row["stream_bitrate_kbps"] or 196),
                ),
...
class StationRuntimeRegistry:
    def __init__(self, runtime_factory=None, live_mic_registry=None):
    ...
    def _is_legacy_implicit_icecast_default(cls, row, station_id: int) -> bool:
        ...
        return (
            ...
            and abs(gain) <= 1e-9
            and str(row["stream_codec_profile"] or "aac_plus_196").strip() == "aac_plus_196"
            and int(row["stream_bitrate_kbps"] or 196) == 196
        )
    ...
    def _start_station_unlocked(
        ...
        if crossfade_seconds is None:
            raw_crossfade_seconds = settings.get("default_crossfade_seconds", 0.0)
        else:
