...
class StationWorker:
    def __init__(
        ...
        self.lease_service = LeaseService(self.conn)
    ...
    def _maybe_insert_sweeper_jingle(self) -> bool:
        ...
        # Insert jingle at front of pending queue (position = lowest pending - 1)
        cur = self.conn.cursor()
        cur.execute(
            "SELECT MIN(position) as min_pos FROM queue_items "
            "WHERE station_id=? AND status='pending'",
    ...
    def _select_random_music_track(self, exclude_ids: set[int]) -> dict | None:
        """Pick from the least-played tier using a reproducible station seed."""
        cur = self.conn.cursor()
        ...
        query = (
            ...
            "COALESCE(play_count, 0) AS play_count FROM tracks WHERE "
            + " AND ".join(where)
    ...
    def _autofill_queue(self) -> int:
        ...
        recent_ids = self._recent_track_ids(limit=30) | active_ids
        added = 0
