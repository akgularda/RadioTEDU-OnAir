...
_MAX_FAILURE_BACKOFF_SECONDS = 120.0
_MIN_FAILURE_BACKOFF_SECONDS = 30.0
...
def _failure_backoff_seconds(failure_count: int, interval_sec: float) -> float:
    ...
    delay = max(
        float(interval_sec),
        _MIN_FAILURE_BACKOFF_SECONDS * (2 ** min(count - 1, 3)),
    )
    return min(_MAX_FAILURE_BACKOFF_SECONDS, delay)
