...
def _sync_station_output_from_settings(conn, station_id: int, settings: dict[str, str]) -> dict:
    ...
    current_profile = str(current["stream_codec_profile"]) if current else "aac_plus_196"
...
def legacy_metadata_autofix(payload: MetadataAutofixPayload):
    ...
    if payload.analyze_bpm:
        def _update_missing_bpm() -> int:
            write_cur = conn.cursor()
            write_cur.execute(
                f"UPDATE tracks SET bpm=120 WHERE {where} AND (bpm IS NULL OR bpm<=0) AND LOWER(track_type)='music'",
                params,
            )
            updated = int(write_cur.rowcount)
            conn.commit()
            return updated

        try:
            bpm_updated = int(_run_sqlite_write_with_retry(conn, _update_missing_bpm))
        except sqlite3.OperationalError as exc:
            if "locked" not in str(exc).lower():
                raise
            error_count = 1
            _log.warning(
                "Metadata autofix skipped BPM update because the database stayed locked: station_ids=%s",
                station_ids,
            )
...
def legacy_bpm_analyze(payload: BpmAnalyzePayload):
    ...
    cur.execute(f"SELECT COUNT(*) AS c FROM tracks WHERE {where}", tuple(params))
    analyzed = int(cur.fetchone()["c"])

    update_where = where
    update_params = list(params)
    if payload.only_missing:
        update_where += " AND (bpm IS NULL OR bpm<=0)"
    bpm_updated = 0
    error_count = 0

    def _update_bpm() -> int:
        write_cur = conn.cursor()
        write_cur.execute(f"UPDATE tracks SET bpm=120 WHERE {update_where}", tuple(update_params))
        updated = int(write_cur.rowcount)
        conn.commit()
        return updated
    ...
    try:
        ...
        error_count = 1
        _log.warning(
            "BPM analyze skipped update because the database stayed locked: station_id=%s",
            int(payload.station_id),
        )

    skipped = max(0, analyzed - bpm_updated)
    return {
        "summary": {
            "analyzed": analyzed,
            "bpm_updated": bpm_updated,
...
def _sync_station_library_folder_with_connection(
    ...
    if len(candidates) > _SCANNER_MAX_NEW_FILES:
        raise HTTPException(
            status_code=413,
            detail=(
                f"Folder contains {len(candidates)} audio files; the deterministic sync limit is "
                f"{_SCANNER_MAX_NEW_FILES}. Split the library into smaller station folders."
            ),
        )

    # Validate the whole folder before the first database write. A replace
    # operation must be all-or-nothing and must never activate a corrupt file.
    metadata_by_path: dict[str, dict[str, str | float]] = {}
    unplayable: list[dict[str, str]] = []
    for path in candidates:
        try:
            metadata_by_path[_canonical_library_path(path)] = _get_audio_metadata(
                str(path),
