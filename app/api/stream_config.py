...
class StreamDraftPayload(BaseModel):
    ...
    stream_codec_profile: str = "mp3_128"
...
def apply_stream_draft(
    ...
    if payload.override_reason and "stream.configure_advanced" not in set(user.get("effective_permissions") or []):
        raise HTTPException(status_code=403, detail="advanced_permission_required")
    try:
        return stream_config_service.apply(draft_id, actor_id=int(user["id"]), idempotency_key=idempotency_key, override_reason=payload.override_reason)
