...
class AIHostService:
    """
    AI host that generates announcements using local Qwen models.

    Text generation provider order:
    1. Local Qwen LLM when available
    2. Built-in template rendering

    Speech synthesis provider order:
    1. edge-tts cloud synthesis when available
    2. Local Qwen TTS when available
    """
    ...
    def _store_cached_announcement(self, announcement: AIAnnouncement) -> None:
        ...
        try:
            metadata_path.write_text(
                json.dumps(announcement.to_metadata(), ensure_ascii=True, indent=2),
                encoding="utf-8",
            )
        except Exception:
