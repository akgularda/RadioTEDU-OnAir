...
def _repair_station_output_mount_typos(cur) -> bool:
    """Repair only the known main-station `/radio` typo, never similar mounts."""
    cur.execute(
        "SELECT so.station_id FROM station_outputs so "
        "JOIN stations s ON s.id=so.station_id "
        "WHERE so.icecast_mount='/radio' "
        "AND LOWER(s.name) IN ('radiotedu', 'main radio', 'radiotedu main')"
    )
    station_ids = [int(row[0]) for row in cur.fetchall()]
    for station_id in station_ids:
        cur.execute(
            "UPDATE station_outputs SET icecast_mount='/radiotedu1' "
            "WHERE station_id=? AND icecast_mount='/radio'",
            (station_id,),
        )
        cur.execute(
            "UPDATE station_settings SET value='/radiotedu1' "
            "WHERE station_id=? AND key='icecast_mount' AND value='/radio'",
            (station_id,),
        )
        cur.execute(
            "UPDATE station_settings "
            "SET value=REPLACE(value, '/radio', '/radiotedu1') "
            "WHERE station_id=? AND key='icecast_url' "
            "AND (value LIKE '%/radio' OR value LIKE '%/radio?%')",
            (station_id,),
        )
    return bool(station_ids)
...
def _migrate_station_outputs(cur) -> None:
    ...
    if "stream_codec_profile" not in existing:
        cur.execute(
            "ALTER TABLE station_outputs ADD COLUMN stream_codec_profile TEXT NOT NULL DEFAULT 'aac_plus_196'"
        )
    if "stream_bitrate_kbps" not in existing:
        cur.execute(
            "ALTER TABLE station_outputs ADD COLUMN stream_bitrate_kbps INTEGER NOT NULL DEFAULT 196"
        )
...
def _bootstrap_schema(cur) -> None:
    ...
    cur.execute(
        ...
        "stream_codec_profile TEXT NOT NULL DEFAULT 'aac_plus_196', "
        "stream_bitrate_kbps INTEGER NOT NULL DEFAULT 196"
        ")"
    )
