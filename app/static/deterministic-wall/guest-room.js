(() => {
  ...
  async function refresh() {
    ...
    studioId = nextStudio;
    try { room = await request(`/api/studios/${studioId}/guest-room`); panel.hidden = false; render(); }
    catch (error) { if (Number(error.status || 0) === 403) panel.hidden = true; else result('guestInviteResult', errorMessage(error), 'error'); }
  }

  byId('createGuestInviteButton').addEventListener('click', async () => {
    if (!studioId) return;
    try { const invite = await request(`/api/studios/${studioId}/guest-invites`, { method: 'POST' }); await navigator.clipboard.writeText(invite.join_url); result('guestInviteResult', `Copied a one-time link. It expires at ${new Date(invite.expires_at).toLocaleTimeString()}.`, 'success'); await refresh(); }
  ...
  byId('guestRecordButton').addEventListener('click', async () => { try { const recording = await request(`/api/studios/${studioId}/guest-recordings`, { method: 'POST' }); result('guestRecordingResult', recording.status === 'recording' ? 'Recording started.' : 'Consent requested. Recording will start only after everyone agrees.', 'success'); await refresh(); } catch (error) { result('guestRecordingResult', errorMessage(error), 'error'); } });
  byId('guestStopRecordButton').addEventListener('click', async () => { try { await request(`/api/studios/${studioId}/guest-recordings/${room.recording.id}/stop`, { method: 'POST' }); result('guestRecordingResult', 'Recording stopped and finalized.', 'success'); await refresh(); } catch (error) { result('guestRecordingResult', errorMessage(error), 'error'); } });
