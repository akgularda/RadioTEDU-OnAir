...
app = FastAPI(title="RadioTEDU OnAir API", version="1.0.0", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
...
# remains available from the canonical Broadcast Wall application.
...
def frontend_index():
    return FileResponse(_STATIC_DIR / "lobby.html")


@app.get("/app", include_in_schema=False)
@app.get("/app/", include_in_schema=False)
def frontend_app():
    return FileResponse(_STATIC_DIR / "deterministic-wall" / "index.html")
